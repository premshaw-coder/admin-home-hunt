...
export const authGuard: CanActivateFn = () => {
  ...
  subscriptionsStatusService.getSubscriptionStatus().subscribe((userSubscription: SubscriptionStatus) => {
  ...
  return subscriptionsStatusService.getSubscriptionStatus().pipe(
    map((userSubscription: SubscriptionStatus) => {
      ...
      if (userSubscription?.subscription_status === 'inactive' && authService.isUserAuthenticated()) {
